({
  ...
  onReceiveNotification: function (component, message) {
    ...
    const newNotification = {
      ...
      id: message.data.payload.Identification__c,
      type: message.data.payload.Error_Type__c
    };
    ...
    //this.displayToast(component, 'info', newNotification.message+' ID:'+newNotification.id+' - '+newNotification.type);

    if(newNotification.type === 'error'){
      this.displayToast(component, 'error', newNotification.message+' ID:'+newNotification.id);    
    }else if (newNotification.type === 'warning'){
      this.displayToast(component, 'warning', newNotification.message+' ID:'+newNotification.id);    
    }else{
      this.displayToast(component, 'info', newNotification.message+' ID:'+newNotification.id);
    }
  },
